...
Hermes SKILL.md dosyaları ile Obsidian skill notlarını karşılaştırır.
İsim çakışmalarını tespit eder (Hermes audiocraft = Obsidian audiocraft-audio-generation vb.)
Temiz (shared/hermes-only/obsidian-only) rapor üretir.
...
Output format (JSON):
  {
    ...
    "isim_cakismalari": [{"hermes": "audiocraft", "obsidian": "audiocraft-audio-generation"}, ...],
    "summary": {
      "hermes_count": 142,
      "obsidian_count": 156,
      "shared": 133,
...
# === PATHS ===
HERMES_SKILLS = Path(r"C:\Users\marko\AppData\Local\hermes\skills")
OBSIDIAN_SKILLS = Path(r"C:\Users\marko\OneDrive\Belgeler\Obsidian Vault\Hermes\Skills")

# === KNOWN NAME ALIASES (Hermes adı -> Obsidian ad(lar)ı) ===
...
def get_hermes_skills() -> dict[str, str]:
    """Hermes skills dizinini tara, {normalized_name: real_name} sözlüğü döndür."""
    skills = {}
...
def main():
    hermes = get_hermes_skills()
    obsidian = get_obsidian_notes()

    hermes_norm = set(hermes.keys())
    obsidian_norm = set(obsidian.keys())

    shared_norm = hermes_norm & obsidian_norm
    hermes_only_norm = hermes_norm - obsidian_norm
    obsidian_only_norm = obsidian_norm - hermes_norm

    # Normalden gerçek isimlere çevir
    shared = sorted(hermes[n] for n in shared_norm)
    hermes_only = sorted(hermes[n] for n in hermes_only_norm)
    obsidian_only = sorted(obsidian[n] for n in obsidian_only_norm)

    # İsim çakışmaları: NAME_ALIASES'teki eşleşmeleri kontrol et
    isim_cakismalari = []
    for hermes_name, alias_info in NAME_ALIASES.items():
        hermes_norm_name = normalize(hermes_name)
        if hermes_norm_name not in hermes_norm:
            continue
