...
- **Obsidian log satırı eklerken PowerShell `Add-Content` kullanma** — Türkçe karakterler (ö/ş/ı/ğ/ü/ç) bozulur.
  - **Doğru yöntem:** Obsidian log dosyasını `write_file` ile baştan yaz (önce tüm satırları oku + yeniyi ekle + tümünü yaz).
  - Veya `[System.IO.File]::AppendAllText(path, text, [System.Text.Encoding]::UTF8)` PowerShell ile — ama bash içindeki quote kaosu nedeniyle `write_file` daha güvenilir.
  - ⚠️ **write_file yol formatı:** `write_file` tool'u **native Windows yolu** bekler (`C:\Users\marko\...`). MSYS tarzı `/c/Users/marko/...` yolu verilirse tool `C:\c\Users\marko\...` olarak yanlış çözümler. `terminal()` içinde `/c/Users/...` çalışır ama `write_file`/`read_file`/`patch` tool'larında `C:\Users\...` kullan.
