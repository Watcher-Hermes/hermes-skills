"""
HERMES BACKUP SYNC — no_agent script
...
HERMES_HOME = Path(os.environ.get("HERMES_HOME",
    r"C:\Users\marko\AppData\Local\hermes"))
BACKUP_DIR = Path(os.environ.get("USERPROFILE", r"C:\Users\marko")) / "hermes-backup"
LOG_FILE = HERMES_HOME / "logs" / "backup-sync.log"
...
def main():
    ...
    if not (BACKUP_DIR / ".git").exists():
        ...
        subprocess.run(["git", "clone", "https://github.com/asdafgf/hermes-full-backup.git", str(BACKUP_DIR)], timeout=120)
        log("Repo klonlandi")
        changes = True
    ...
    if HERMES_SKILLS.exists() and REPO_SKILLS.exists():
        local_skills = dir_manifest(HERMES_SKILLS)
        ...
        if added or modified:
            ...
            shutil.copytree(HERMES_SKILLS, REPO_SKILLS,
    ...
    REPO_MEMOR = BACKUP_DIR / "Hermes Memor"
    if MEMORIES.exists():
    ...
    PART1 = BACKUP_DIR / "hermes-state-part001.zip"
    PART2 = BACKUP_DIR / "hermes-state-part002.zip"
    ...
    if STATE_DB.exists():
        ...
        if size_diff > 1_000_000 or (mtime_diff < 3600 and size_diff > 100_000):
            zip_path = BACKUP_DIR / "hermes-state-full.zip"
