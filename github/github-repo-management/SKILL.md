...
## 11. Pre-Install Evaluation (Third-Party Repos)

Before installing any third-party GitHub repo into Hermes (skills, tools, or integrations), run this evaluation:

### Security Pre-Check

```bash
# 1. Check scripts for dangerous patterns
#    Look for: eval(), exec(), os.system(), subprocess without sandbox,
#              curl/wget to unverified URLs, embedded tokens/passwords

# 2. Check requirements.txt / package.json
#    - Are dependencies legitimate? (known packages, not typosquats)
#    - Any suspicious package names?

# 3. Verify upstream integrity (forks)
#    Compare key files SHA with upstream:
...
```bash
# CAUTION: npx opens interactive prompt on Windows and may hang
# Use --yes flag or pipe to avoid interactive mode
npx skills add owner/repo --yes
# Limitations: only finds skills in .claude/skills/ at repo root,
