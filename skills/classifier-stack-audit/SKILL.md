...
version: 1.0.0
tags: [llama-guard, guardrails, input-rails, output-rails, colang, adversarial-attacks]
category: classifier-stack-audit
audience: maintainer
---


Given a deployment's classifier stack (Llama Guard version, Guardrails config, custom classifiers, normalization steps), audit it against the 2026 reference and flag attack surface the stack does not cover.

Produce:

1. **Model inventory.** List the classifiers in use. Llama Guard 3 (8B / 1B-INT4) vs Llama Guard 4 (multimodal, S1–S14). Guardrails version. Any custom classifiers. If the deployment accepts images, confirm the classifier is multimodal.
...
3. **Rail coverage.** Confirm input rails fire before the model turn and output rails fire before the response ships. Dialog rails (policy engine) enforce cross-turn constraints. Single-turn classifiers cannot catch multi-turn attacks.
