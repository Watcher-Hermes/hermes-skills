...
1. Five-layer pipeline: input sanitize (zero-width strip, encoding decode, Unicode normalize) -> Guardrails v0.12 rails -> classifier gate (Llama Guard 4 / X-Guard / ShieldGemma-2 / custom classifier) -> target LLM -> output filter (Llama Guard 4 + Presidio PII + citation check). Flagged outputs go to a Slack HITL queue.
